...
public class characterUtility : MonoBehaviour {
	...
	public int castCapsule()
	{
		...
		if (Physics.CapsuleCast (s1, s2, controller.radius * 2, transform.forward, out hit, Vector3.Distance (s1, s2)) && hit.collider.tag == "moveit") {
	...
	void Update () 
	{
		...
		if (holdingState == 1 && Input.GetKeyDown (KeyCode.Z)) {
			// we push the object here	

			hit.rigidbody.position += FORWARD;
		...
		if (holdingState == 2) {
			...
			ball.gameObject.rigidbody.position = handSimulation;
